...
select b.requirementname,
       ...
       CAST(
               CASE
                   ...
                   WHEN ( select  count(tt.*) from  ehr_compliancedb.requirements tt, ehr_compliancedb.completiondates pq where tt.requirementname = b.requirementname and pq.requirementname = tt.requirementname and pq.employeeid = a.employeeid group by tt.expireperiod
                          having (tt.expireperiod) >  (age_in_months(tt.reviewdate, max(pq.date)) and (tt.reviewdate is not null)  )) > 0 THEN

                       ( select  (age_in_months(tt.reviewdate, max(pq.date)) - ( age_in_months(max(pq.date), curdate())) )from  ehr_compliancedb.requirements tt, ehr_compliancedb.completiondates pq where tt.requirementname =   a.requirementname and pq.requirementname = tt.requirementname and pq.employeeid = a.employeeid group by tt.expireperiod
...
select a.requirementname,
    ...
    CAST(
               CASE
                   ...
                   WHEN ( select  count(tt.*) from  ehr_compliancedb.requirements tt, ehr_compliancedb.completiondates pq where tt.requirementname = a.requirementname and pq.requirementname = tt.requirementname and pq.employeeid = a.employeeid group by tt.expireperiod
                          having (tt.expireperiod) >  (age_in_months(tt.reviewdate, max(pq.date)) and (tt.reviewdate is not null)  )) > 0 THEN
