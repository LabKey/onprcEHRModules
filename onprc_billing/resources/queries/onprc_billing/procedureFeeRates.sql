...
SELECT
    ...
    round(CAST(CASE
                   ...
                   --raise F&A only
                   --WHEN (alias.aliasType.removeSubsidy = false AND (alias.aliasType.canRaiseFA = true AND p.chargeId.canRaiseFA = true)) THEN (cr.unitCost * (CASE WHEN (alias.faRate IS NOT NULL AND alias.faRate < CAST(ir.indirectRate AS DOUBLE)) THEN (1 + (CAST(ir.indirectRate   AS DOUBLE) - alias.faRate)) ELSE 1 END))
    ...
    1 as quantity,
    COALESCE(cu.account, cast(ce.account as varchar(200))) as creditAccount,
...
FROM onprc_billing.procedureFees p
    ...
    --NOTE: the objective is to credit assisting staff for this service when research staff is the primary
    (CASE WHEN (p.chargetype = 'Research Staff') THEN p.assistingstaff ELSE p.chargetype END) = cu.chargetype AND
    cast(cu.startDate AS date) <= cast(p.date as date) AND
    cast(cu.endDate AS date) >= cast(p.date as date)
    )
    /*Added to use Indirect Rates*/
