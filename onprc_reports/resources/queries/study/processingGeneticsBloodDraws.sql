...
SELECT
  ...
  CASE
    ...
    WHEN (LOWER(d.species) = 'rhesus macaque' AND d.Id.age.ageInYears <= 5.0 AND d.geographic_origin = 'India' AND m.Id IS NULL AND (a.Id IS NOT NULL OR LOWER(d.gender) = 'm' ) and u.id is null ) THEN 1
...
LEFT JOIN (
  ...
  FROM (
    ...
    HAVING (
      (LOWER(s.sampletype) = 'buffy coat' AND sum(coalesce(s.quantity, 0)) >= 5.0) OR
      (LOWER(s.sampletype) = 'whole blood' AND sum(coalesce(s.quantity, 0)) >= 5.0)
    )
  ) t
