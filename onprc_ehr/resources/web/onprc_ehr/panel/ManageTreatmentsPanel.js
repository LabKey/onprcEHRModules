...
Ext4.define('onprc_ehr.panel.ManageTreatmentsPanel', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.onprc_ehr-managetreatmentspanel',
    ...
    statics: {
            ...
            return [{
                ...
                menu: [{
                    ...
                    handler: function(btn){
                        onprc_ehr.panel.ManageTreatmentsPanel.createNewTreatmentWindow(owner, btn, 'Clinical');
                    }
                },{
                    text: 'Surgical Treatment',
                    handler: function(btn){
                        onprc_ehr.panel.ManageTreatmentsPanel.createNewTreatmentWindow(owner, btn, 'Surgical');
                    }
                }]
            },{
                xtype: 'button',
        ...
        createNewTreatmentWindow: function(owner, btn, category){
            onprc_ehr.panel.ManageTreatmentsPanel.createTreatmentWindow(btn, {
                listeners: {
                    scope: this,
                    save: function(){
                        ...
                        var panel;
                        if (win){
                            panel = win.down('onprc_ehr-managetreatmentspanel');
                        }
                        else {
                            panel = btn.up('onprc_ehr-managetreatmentspanel');
                        }
        ...
        getActionBtnMenu: function(rec){
            return Ext4.create('Ext.menu.Menu', {
                items: [{
                    ...
                    text: 'Edit Treatment',     //Modified: 8-29-2016  R.Blasa
                    disabled: !EHR.Security.hasPermission(EHR.QCStates.COMPLETED, 'update', [{schemaName: 'study', queryName: 'Treatment Orders'}]),
                    handler: function(btn){
                        onprc_ehr.panel.ManageTreatmentsPanel.createTreatmentWindow(btn, {
                            pkCol: 'objectid',
        ...
        return {
            ...
            columns: [{
                ...
                handler: function(view, rowIndex, colIndex, item, e, rec){
                    onprc_ehr.panel.ManageTreatmentsPanel.getActionBtnMenu(rec).showAt(e.getXY());
                }
            },{
                header: 'Date',
                xtype: 'datecolumn',
