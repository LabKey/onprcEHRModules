 ...
 */
Ext4.define('ONPRC_EHR.window.CopyPathRequestWindow', {
    ...
    getSearchItems: function(){
        return [{
            html: 'This helper allows you to copy a set of Path tissues previously used in any necropsy.  You can search by project, animal or recipient.  When you search, it will return all distinct sets of tissues matching your criteria, and you can choose which to use in this form.',
    ...
    onSubmit: function(){
        ...
        if (!animal && !project && !recipient){
            Ext4.Msg.alert('Error', 'Must enter either an animal Id');
            return;
        }

        var filterArray = [];

        //Default to Pathology Tissue entries
        filterArray.push(LABKEY.Filter.create('type', 'Tissue', LABKEY.Filter.Types.EQUAL));
...
EHR.DataEntryUtils.registerGridButton('COPY_PathTissues', function(config){
    config = config || {};

    return Ext4.Object.merge({
        text: 'Copy Previous Path Tissues',
        itemId: 'copyTissues',
        handler: function(btn){
            var grid = btn.up('gridpanel');
            LDK.Assert.assertNotEmpty('Unable to find gridpanel in COPY_TISSUES button', grid);

            Ext4.create('ONPRC_EHR.window.CopyPathRequestWindow', {
