...
Ext4.define('ONPRC_EHR.window.PathTissuesImportWindow', {
    ...
    initComponent: function(){
        Ext4.apply(this, {
            modal: true,
            closeAction: 'destroy',
            title: 'Import Path Tissues Results From Excel',
            ...
            items: [{
                html: 'This helper allows you to bulk import Path Tissues data, exported as an excel file.',
...
EHR.DataEntryUtils.registerGridButton('PathTissues_IMPORT', function(config){
    return Ext4.Object.merge({
        ...
        handler: function(btn){
            ...
            LDK.Assert.assertNotEmpty('Unable to find grid in EPOC_IMPORT button', grid);

            var panel = grid.up('ehr-dataentrypanel');
            LDK.Assert.assertNotEmpty('Unable to find dataEntryPanel in PathTissues_IMPORT button', panel);

            var runStore = panel.storeCollection.getClientStoreByName('Tissue_distrubutions');
