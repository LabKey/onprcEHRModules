 ...
 * @cfg dataEntryPanel
 */
// Created 8-12-2025  Blasa    Bulk Clinical Screen Mass processing

Ext4.define('ONPRC_EHR.window.ClinicalProcessingTPRWindow', {
    ...
    initComponent: function(){
        Ext4.apply(this, {
            modal: true,
            closeAction: 'destroy',
            title: 'Clinical Processing TPR Import',
            ...
            items: [{
                html : 'This allows you to import record using the Clinical Processing TPR Template excel form.  To import, cut/paste the contents of the excel file (Ctl + A is a good way to select all) into the box below and hit submit.',
    ...
    processRow: function(row, recordMap, errors, rowIdx, chargeunit, date, projects,times,parsed, performedBy, procedureRec)
    {
        ...
        if (row[0] && row[8] && row[9]){
            var obj = {
                ...
                observation:this.safeGet(parsed, 50, 1),  // Alopecia Regrowth
                performedby: performedBy
...
EHR.DataEntryUtils.registerDataEntryFormButton('CLINICPROC_TPR_IMPORT', {
    ...
    handler: function(btn){
        ...
        LDK.Assert.assertNotEmpty('Unable to find dataEntryPanel in Clinical Processing TPR Import button', panel);

        Ext4.create('ONPRC_EHR.window.ClinicalProcessingTPRWindow', {
