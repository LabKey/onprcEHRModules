...
SELECT
  ...
  CASE
    ...
    WHEN (pc.cage_type.height < c1.height AND (f.heightExemption IS NULL AND wf.weightExemption iS NULL ) )THEN ('ERROR: Insufficient height, ' || h.id ||' needs at least: ' || cast(c1.height AS varchar(50)))
