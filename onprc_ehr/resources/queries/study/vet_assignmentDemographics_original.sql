SELECT
    ...
    h.room,
--d.calculated_status,
/*    Case UPDATE NO Longer evaluate Deceased of Shipped NHPs
...
--n.assignedVet as DeceaseDAssignedVet,
    ...
    c.category,
    CASE
        --  When cr.id is null then 'No Clin Remarks Found' UPDATE NO longer handle deceased or shipped
        ...
        WHen r1.id is not null then r1.projecttype -- changed to project Type
        WHen r2.id is not null then r2.projecttype  -- changed to project type
    ...
    Case
        ...
        when r1.project is not null then r1.use_Category --changed protocol to project
        when r2.project is not null then r2.use_category --changed protocol to project
        else  ' '
        End As use_category,
--r1.use_Category,
--This needs to be a case statement that resturned the protocol whether Resource or research with research being the priority
    ...
    /*  Case == Update Np longer evaluating  protocol
  ...
  */
    ...
    /*  Case -- UPDATE No longer evaluation Protocol Elements
...
/* --UPDATE We nop longer need to handle deceased or shipped NHPs
LEFT JOIN (
...
--Review for Assigned Animals
    ...
    -- No evaluationg depautre or deaths
...
where
    ...
    (d.demographics.calculated_status is not null) --and
-- d.id not Like '[A-Z]%'
