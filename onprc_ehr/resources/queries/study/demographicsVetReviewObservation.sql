select dem.*,
       ( select group_concat(DISTINCT j.remark, chr(10))  from "Clinical Observations" j where j.id = dem.id  and j.remark is not null and j.date >= (select max(k.date)
...
Where
        dem.id in (Select cln.id from "Clinical Observations" cln where cln.category = 'Observation' and cln.remark is not null
