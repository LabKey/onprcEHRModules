...
select
  ...
  (SELECT group_concat(DISTINCT f.flag.value) AS v FROM study.flags f WHERE f.id = d.id AND (f.flag.value IN ('DO NOT Administer intradermal tuberculin test', 'Administer TB test in left eyelid ONLY')) AND f.isActive = true) as flags,
