...
WHERE p.qcstate.publicdata = true
  ...
  And p.date in (Select max(r.date) from study.Clinical_Observations r, study.Weight j
                 Where r.category = 'BCS' And r.id = p.id And r.id = j.id And r.QCState.Label = 'Completed'
                   And (TIMESTAMPDIFF('SQL_TSI_DAY', j.date, r.date) < 8) And r.qcstate = 18 And j.QCState.Label = 'Completed')
