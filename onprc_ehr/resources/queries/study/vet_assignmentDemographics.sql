...
LEFT JOIN Site.{substitutePath moduleProperty('EHR', 'EHRStudyContainer') }.study.ClinicalCases_Open AS CMUcase ON CMUcase.id = nhp.id
    ...
    LEFT JOIN Site.{substitutePath moduleProperty('EHR', 'EHRStudyContainer') }.study.assignment assignment ON assignment.id = nhp.id
...
WHERE demographics.Calculated_Status = 'Alive'
  ...
  AND (Assignment.enddate is null or assignment.enddate >= now())
