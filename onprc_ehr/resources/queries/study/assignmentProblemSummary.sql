...
SELECT
  t1.project,
  t1.category,
  (select count(distinct m.id) AS Ids FROM study.assignment m WHERE (m.date <= max(t1.EndDate) AND m.enddateCoalesced >= max(t1.StartDate) AND m.project = t1.project) GROUP BY m.project) as totalIds,
